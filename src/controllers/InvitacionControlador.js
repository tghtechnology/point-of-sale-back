...
export const enviarInvitacion = async (req, res) => {
  ...
  try {
    const info = await InvitacionServicio.enviarCorreoInvitacion(emailDestinatario);
    res.status(200).json({ mensaje: 'Correo de invitación enviado correctamente', info });
  } catch (error) {
